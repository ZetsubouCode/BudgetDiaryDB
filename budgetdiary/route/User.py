...
async def update_by_id(id: int, discord_username:Optional[str] = Form(None), pin: Optional[str] = Form(None), balance:float = Form(...)):
...
async def update_by_username(discord_username: str, pin:Optional[str] = Form(None), balance:float = Form(...)):
...
async def delete_by_username(target_username: str):
    ...
    try:
        user = await UserController.get_by_username(target_username)
        if not user:
            Debug.msg(debug_identifier, "User does not exist")
            return BaseResponse(**{"status": "User does not exist"})

        success_delete = await UserController.delete_by_username(target_username)
        if not success_delete:
            Debug.msg(debug_identifier, "Failed to delete data")
            return BaseResponse(**{"status": "Error", "content": "Failed to delete data"})
